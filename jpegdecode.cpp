...
Mat Jpeg2Mat(uint8_t *jpegData, uint64_t jpegSize)
{
    ...
    if ((decoder = gpujpeg_decoder_create(0)) == nullptr) {
            return result;
    }
    gpujpeg_decoder_set_output_format(decoder, GPUJPEG_RGB, GPUJPEG_444_U8_P012);

    // decompress the image
    uint8_t *image_decompressed = nullptr;
    int image_decompressed_size = 0;
    if (gpujpeg_decoder_decode(decoder, jpegData, jpegSize, &decoder_output) != 0) {
            return result;
    }
    int width = 2560;
    int height = 960;
    Mat dst(height,width,CV_8UC3,(uint8_t*)decoder_output.data);
